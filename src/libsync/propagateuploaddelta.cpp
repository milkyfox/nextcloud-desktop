...
void PropagateUploadFileDelta::slotBlockUploaded()
{
    ...
    if (httpCode == 412) {
        slotJobDestroyed(job);
        job->deleteLater();
        // Another client changed the file concurrently: our block map and etag are stale.
        // Force remote re-discovery so the next sync round fetches a fresh block map,
        // and clear the etag so any fallback path does not send a stale If-Match.
        propagator()->_journal->schedulePathForRemoteDiscovery(_item->_file);
        propagator()->_anotherSyncNeeded = true;
        _item->_etag.clear();
        done(SyncFileItem::SoftError, tr("File changed on the server during delta sync; it will be retried."));
        return;
    }
